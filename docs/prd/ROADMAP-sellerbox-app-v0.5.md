...
- **배송 관리**: 배송관리 통합 화면(배송출발·완료·BBQ 배송지별 묶음) + 배송현황판 실시간 모니터링 (Supabase Realtime)
...
- ✅ **fix: store.delivery_dddress 오타 컬럼 정리** - 완료

  #### 요구사항
  - DB `store` 테이블에 `delivery_dddress`(오타)와 `delivery_address`(정상) 두 컬럼이 공존 → 오타 컬럼 삭제

  #### 수정 파일
  - `lib/supabase/database.types.ts`: `store` 타입 `delivery_dddress` → `delivery_address` 수정
  - `lib/schemas/domain/store.schema.ts`: `delivery_dddress` → `delivery_address` 수정
  - `app/(admin)/stores/_components/store-detail-form.tsx`: 폼 필드 key 3개소 수정
  - `lib/mocks/store.ts`: mock 데이터 key 5개소 수정
  - `supabase/migrations/20260419000002_rename_delivery_dddress.sql` 신규: `ALTER TABLE store DROP COLUMN IF EXISTS delivery_dddress`

- ✅ **Task 041: 상품 카테고리 common_code DB 기반 동적 로드 (F001)** - 완료

  #### 요구사항
  - 상품 등록/수정 팝업 및 조회 필터의 카테고리 드롭다운을 하드코딩 배열 → DB `common_code` / `common_code_value` 기반 동적 로드로 전환
  - 카테고리 조회 체인: 가게 선택 → `store.tenant_id` → `tenant.code` → `common_code.code` → `common_code_value` records

  #### 신규 파일
  - `lib/actions/domain/category.actions.ts`: `getCategoriesByCode(storeCategoryCode)` 서버 액션
  - `lib/hooks/use-category-options.ts`: `useCategoryOptions(storeCategoryCode)` 클라이언트 훅 — 가게 변경 시 자동 재로드, cancelled flag cleanup

  #### 수정 파일
  - `lib/supabase/database.types.ts`: `common_code`, `common_code_value` 테이블 타입 추가
  - `components/admin/domain/category-select.tsx`: `CategoryOption` 타입 export 추가
  - `app/(admin)/items/page.tsx`: store 조회에 `tenant_id` 포함 + `tenant.code` 조인 → stores에 `tenant_code` 포함 전달
  - `app/(admin)/items/items-client.tsx`: `useCategoryOptions` 훅 적용, 가게 변경 시 카테고리 필터 자동 초기화

- ✅ **Task 042: 카테고리 드롭다운 common_code DB 동적 로드 전면 적용 (F001-2·F002·F014·F015)** - 완료

  #### 요구사항
  - Task 041의 `useCategoryOptions` 훅을 상품설명관리·재고관리·프로모션 화면에 전면 적용
  - 프로모션 가게 상품목록 그리드 카테고리 컬럼: `category_code_value`(코드 키) → `category_name`(한글명) 수정

  #### 수정 파일
  - `app/(admin)/items/detail/page.tsx`: tenant_code 조인 포함 stores 전달
  - `app/(admin)/items/detail/item-detail-client.tsx`: `useCategoryOptions` 훅 적용, 가게 변경 시 카테고리 초기화
  - `app/(admin)/inventory/page.tsx`: tenant_code 조인 포함 stores 전달
  - `app/(admin)/inventory/inventory-mgmt-client.tsx`: `useCategoryOptions` 훅 적용, 가게 변경 시 카테고리 초기화 (`withUnsavedGuard` 내부)
  - `app/(admin)/promotions/_components/item-select-grid.tsx`: 카테고리 컬럼 key `category_code_value` → `category_name`

  #### 검증 결과 (Playwright MCP)
  - `/items` 상품 조회/목록 — 카테고리 필터 11개 항목 정상 로드 ✅
  - `/items` 상품 등록 팝업 — 카테고리 드롭다운 11개 항목 정상 로드 ✅
  - `/items/detail` 상품설명관리 — 카테고리 필터 11개 항목 정상 로드 ✅
  - `/inventory` 등록상품 재고관리 — 카테고리 필터 11개 항목 정상 로드 ✅
  - `/promotions` 프로모션 관리 — 가게 상품목록 카테고리 컬럼 한글명 정상 표시 ✅
