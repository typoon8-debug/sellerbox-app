...
> **구현 기능:** `F012` | **인증:** OWNER·MANAGER

| 항목 | 내용 |
|------|------|
| **역할** | 테넌트별 가게 기본정보 등록·수정·삭제 |
| **진입 경로** | 메뉴 → 매장 관리 → 가게관리 클릭 |
| **사용자 행동** | 가게 목록 조회, 신규 가게 등록, 기존 가게 수정·삭제 |
| **주요 기능** | • 가게 목록 테이블 (가게명·카테고리·상태·계약기간)<br>• 가게 등록 폼 (이름·주소·전화번호·카테고리·계약정보)<br>• 가게 상태 변경 (ACTIVE·INACTIVE·CLOSED·PENDING)<br>• **가게 등록** / **수정** / **삭제** 버튼 |
| **다음 이동** | 저장 성공 → 가게 목록 갱신, 실패 → 오류 메시지 표시 |

---

### 가게정보관리 페이지

> **구현 기능:** `F013` | **인증:** OWNER·MANAGER

| 항목 | 내용 |
|------|------|
| **역할** | 매장 운영에 필요한 상세 설정(배송·판매원·바로퀵·슬롯)을 탭별로 관리 |
| **진입 경로** | 메뉴 → 매장 관리 → 가게정보관리 클릭 |
| **사용자 행동** | 탭 전환 후 해당 정보 등록·수정·삭제 |
| **주요 기능** | • [배송정보 탭] 가게별 배송정보 등록/수정/삭제<br>• [판매원정보 탭] 판매원 계정 등록/수정/삭제 (role: PICKER·PACKER 포함)<br>• [바로퀵정책 탭] 최소주문금액·일일운행횟수·슬롯용량·상태 설정<br>• [바로퀵고정운행표 탭] 출발시간·요일마스크·오더컷오프 설정<br>• [슬롯예약카운트 탭] 날짜·시간별 예약 건수 조회 및 설정<br>• **저장** / **삭제** 버튼 |
| **다음 이동** | 저장 성공 → 해당 탭 데이터 갱신, 실패 → 오류 메시지 표시 |
...
| **진입 경로** | 메뉴 → 상품 관리 → 상품관리 클릭 |
| **사용자 행동** | 상품 목록 조회, 카테고리·상태 필터링, 상품 등록·수정·삭제 |
| **주요 기능** | • 상품 목록 테이블 (카테고리·상품명·정가·판매가·랭킹·상태)<br>• 상품 등록/수정 폼 (SKU·카테고리·이름·가격·이미지·랭킹)<br>• 상품 상태 변경 (ACTIVE·INACTIVE·OUT_OF_STOCK·DISCONTINUED)<br>• 상품 상세 이미지 업로드 (Supabase Storage)<br>• **상품 등록** / **수정** / **삭제** 버튼 |
