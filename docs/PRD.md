...
🛒 주문 처리
├── 주문처리 (피킹/패킹/라벨/배송) — F003+F004+F005+F006+F007 통합 (화면번호: 31010)
│   ├── [Panel 1] 주문목록 (체크박스 다중선택 + 배송구분 필터 + 상태 인라인 수정)
│   ├── [Panel 2] 주문상세 (order_item 목록 + 재고수량 부족 경고)
│   ├── [Panel 3] 주문처리 (피킹·패킹·라벨출력 버튼 + 처리결과 테이블)
│   └── [Panel 4] 배송요청 (dispatch_request 목록)
├── 피킹 작업 관리 — F003
├── 패킹 작업 관리 — F004
├── 라벨 관리 — F005
└── 피킹/패킹리스트 출력 — F006
